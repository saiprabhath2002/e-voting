...
def getOutput(status):
    ...
    con = pymysql.connect(host='127.0.0.1',port = 3306,user = 'django', password = '1234', database = 'evoting',charset='utf8')
    with con:
        cur = con.cursor()
...
def ValidateUser(request):
    if request.method == 'POST':
        ...
        ###############
        if len(faces)==0:
        	context= {'data':"can not detect your face please capture correctly"}
        	return render(request, 'UserScreen.html', context)
        ##############
...
def AddPartyAction(request):
    if request.method == 'POST':
      ...
      db_connection = pymysql.connect(host='127.0.0.1',port = 3306,user = 'django', password = '1234', database = 'evoting',charset='utf8')
...
def saveSignup(request):
    if request.method == 'POST':
        ...
        return HttpResponse("Post Incomplete")
